...
## Real Cross-Branch Benchmark (main vs opt1 vs opt2)

**Date**: 2026-03-10  
**Command measured**: `uv run python scripts/run_python_scores.py`  
**Harness**: `uv run python scripts/benchmark_branches.py --branches main opt1 opt2 --warmups 1 --iterations 3`  
**Raw data**: `comparative/branch_benchmark_20260310T221208Z.json`

### Method used for reliable timing

- Isolated `git worktree` per branch (no dirty-working-tree contamination)
- Fresh `uv sync` per worktree
- One warmup run per branch
- Three measured runs per branch
...
|---|---:|---:|---:|---:|---:|---:|---:|
| opt2 | ff1f848 | 6.388 | 0.036 | 6.366 | 6.430 | 6.368 | 1.00x |
| opt1 | c2322b6 | 9.789 | 0.211 | 9.616 | 10.024 | 9.727 | 1.53x slower |
| main | 39ecc73 | 19.632 | 0.098 | 19.526 | 19.720 | 19.650 | 3.07x slower |

### Interpretation

- `opt2` is clearly fastest in this real branch-to-branch run.
- `opt1` improves substantially over `main`, but remains significantly slower than `opt2`.
- The branch gaps are large enough to be robust against run-to-run noise in this setup.

## ASV Adoption (recommended)

ASV is appropriate here for longitudinal tracking and regression detection across commits.

- Added dev dependency: `asv` (in `pyproject.toml` and `uv.lock`)
- Added config: `asv.conf.json`
- Added benchmark target: `benchmarks/time_run_python_scores.py`
...
> Note: in this environment, `uv run asv ...` may fail to resolve the executable; `uv run python -m asv ...` is the reliable invocation.
